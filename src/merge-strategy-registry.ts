...
import { MergeStrategy, MergePlugin, MergeContext, MergeResult, MergeStrategyConfig, FilePatternMergeStrategy } from './types';
...
export class MergeStrategyRegistry {
  ...
  private loadBuiltinStrategies(): void {
    ...
    const builtinStrategies = [
      new OverwriteMergeStrategy(),
      new SkipExistingMergeStrategy(),
      new SimpleMergeStrategy(),
      new JsonMergeStrategy(),
      new PackageJsonMergeStrategy(),
      new MarkdownMergeStrategy(),
  ...
  async resolveStrategy(config: MergeStrategyConfig): Promise<MergeStrategy> {
    switch (config.type) {
      ...
      case 'skip-existing':
        return this.getStrategy('skip-existing') || new SkipExistingMergeStrategy();
